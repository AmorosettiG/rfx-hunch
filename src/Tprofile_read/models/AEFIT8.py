...
# 
#  IDEA:   create a clusterization in latent space using k-means to weight differently the internal representation cases
# 
...
class AEFIT8(models.base.VAE):
    ...
    def __init__(self, feature_dim=40, latent_dim=2, dprate=0., activation=tf.nn.relu, beta=1., 
                 geometry=[20,20,10], scale=1, k_clusters=None, *args, **kwargs):
        ...
        # K-means
        from sklearn.cluster import KMeans
        if k_clusters is None: k_clusters = 2*latent_dim
        self.kms = KMeans(n_clusters=k_clusters, random_state=0)        
